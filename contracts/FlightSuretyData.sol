...
uint256 private enabled = block.timestamp;
  ...
  uint constant MULTI_CONSENSUS_COUNT = 1;
...
// Flights
  ...
  */
  ...
  modifier requireHasAirlinePaid(address airline) {
    require(this.isPaidAirline(airline), "Only existing and paid airlines are allowed to participate");
    _;
  }
  ...
  modifier requireIsContractAuthorized() {
    require(authorizedContracts[msg.sender] == 1, "Not an authorized contract");
    _;
  }  
...
{
    require(pendingPayments[passenger] > 0, "Fund is not enought for withdrawal");
    _;
}
...
modifier isDupCall()
{
   bool isDuplicate = false;
    ...
    for(uint i = 0; i < multiConsensus.length; i++) 
    {
 ...
 function isInsured(bytes32 key, address passenger) external view returns (bool) 
 {
   ...
   */
  function registerAirline(string  name, address addr) external 
   ...
   */   
  function payAirline(address addr) external requireIsOperational requireIsContractAuthorized {
   ...
   */
  function registerFlight(address airline, string  flight, string  from, string  to, uint256 timestamp, bytes32 key) 
  ...
  require(!flights[key].isRegistered, "AirlineFlight is registered already");
   ...
   */   
  function purchase(bytes32 key, address passenger, uint256 amount, uint256 multiplier) external requireIsOperational requireIsContractAuthorized {   

    passengersInsurancePerFlight[key].push(Insurance({
      isCredited: false,
      amount: amount,
  ...
  requireIsContractAuthorized {
    ...
    for (uint index = 0; index < passengersInsurancePerFlight[flightKey].length; index++) 
    {
  ...
  function processFlightStatus(bytes32 flightKey, uint8 statusCode) external requireIsOperational requireIsContractAuthorized {    
